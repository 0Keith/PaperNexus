...
- **Easter eggs (`Core/EasterEggs.cs`):** All hidden UI flourishes live here as pure functions so they are unit tested and cannot touch switching, downloading, or settings. Currently: Konami code in the settings window, version-label ×5 in the footer (cycles messages), occasional alternate splash line, favorite-count milestones, and a few word-spelling annotation hex colours (`#C0FFEE`, `#BADA55`…). **Check this file before adding one** - the Konami egg already existed in `MainWindow.axaml.cs` and was nearly duplicated.
