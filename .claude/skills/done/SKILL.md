...
12. **Parallel doc syncs**: Launch **three** agents simultaneously in a **single message**. Each agent reads the PR diff and updates its target file if needed, then commits and pushes. Use `subagent_type: "general-purpose"` for each.
...
13. **Parallel maintenance**: Launch **two** agents simultaneously in a **single message**. Use `subagent_type: "general-purpose"` for each.
...
14. **Update PR description**: `gh pr edit --body` to reflect final state of all changes. Keep `## Summary` and `## Test plan` format.

15. **Wait for CI**: Poll `gh pr checks`. If failures, fix, commit, push, and re-poll.

16. **Merge**: `gh pr merge --squash --delete-branch`. Fallback to `--merge`, then `--rebase`.

17. **Clean up locally**: Switch to default branch, pull, delete feature branch. Prune remotes (`git fetch --prune`) and delete local branches merged to default. Report cleaned branches.

18. **Verify deploy**: Check for CI/CD on default branch (`gh run list --branch <default> --limit 1`). Watch with `gh run watch`. On failure: read logs, create fix branch, restart from step 1. Skip if no deploy workflow.

19. **Report**: Summarize what merged, review fixes applied, and the PR URL.

20. **Celebrate**: Draw a funny ASCII art of a pokemon saying random phrase like "good job" or "job's done".
